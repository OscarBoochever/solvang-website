...
export default function AdminDepartments() {
  ...
  const loadEntries = async () => {
    const res = await fetch('/api/admin/content?type=department')
                ...
                return (
                  <tr key={entry.sys.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4">
                      <div className="font-medium text-gray-800">{fields.name?.['en-US']}</div>
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-500">
                      /departments/{fields.slug?.['en-US']}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-500">
                      {fields.phone?.['en-US']}
                    </td>
                    <td className="px-6 py-4 text-right">
                      <Link
                        href={`/admin/departments/${entry.sys.id}`}
                        className="text-navy-600 hover:text-navy-800 text-sm font-medium mr-4"
                      >
                        Edit
                      </Link>
                      <button
                        onClick={() => handleDelete(entry.sys.id)}
                        disabled={deleting === entry.sys.id}
                        className="text-red-600 hover:text-red-800 text-sm font-medium disabled:opacity-50"
                      >
                        {deleting === entry.sys.id ? 'Deleting...' : 'Delete'}
                      </button>
                    </td>
                  </tr>
                )
