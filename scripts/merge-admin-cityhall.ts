...
async function main() {
  ...
  // Update City Hall with merged content
  const cityHallEntries = await environment.getEntries({
    content_type: 'department',
    'fields.slug': 'city-hall',
  })

  if (cityHallEntries.items.length > 0) {
    let entry = cityHallEntries.items[0]
        ...
        content: [
          {
            ...
            content: [
              {
                nodeType: 'text',
                value: 'City Hall is the center of Solvang\'s municipal government, housing the City Manager, City Clerk, and Finance departments. Visit us for permits, licenses, public records, payments, and general city services.',
                marks: [],
                data: {},
              },
            ],
        ...
        content: [
          ...
          },
          {
            ...
            content: [
              ...
              { nodeType: 'text', value: ' - Request city documents under the California Public Records Act.', marks: [], data: {} },
            ],
          },
          {
            nodeType: 'paragraph',
    ...
    await entry.publish()
    console.log('Updated City Hall with merged content')
  }

  // Delete Administration department
  const adminEntries = await environment.getEntries({
    content_type: 'department',
    'fields.slug': 'administration',
  })

  if (adminEntries.items.length > 0) {
    const adminEntry = adminEntries.items[0]
    if (adminEntry.isPublished()) {
      await adminEntry.unpublish()
    }
    await adminEntry.delete()
    console.log('Deleted Administration department')
  }
}
